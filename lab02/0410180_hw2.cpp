...
inline int fork_pcs(pid_t* pcs,int index){
	index--;
	//cout<<"index : "<<index ;//<<"  pcs[0] : "<<getpid()<<endl;
	//cout<<"index = :"<<index<<"  pcs[index] = "<<pcs[index]<<" parent id :"<<getppid()<<"  myid :"<<getpid()<<endl;
	if(index > 0 ){
		
		//cout<<"fork pcs index : "<<index<<endl;
		pcs[index] = fork();
		if( pcs[index]>0 ){
	...
	} //if(index >= 0 && pcs[index]==0){
		
		
	//} else {cout<<"             test";return  pcs[0] = fork();}
}
...
int main(int argc ,char**  argv){
	...
	unsigned int temp_ = 0 ; //test
	//unsigned int* Matrix;
	int Matrix_Size;
	
	
	
	cout<<"Input The Matrix Dimension : ";//<<Matrix_Size; //<<Matrix_Size<<endl<<endl;
	cin >> Matrix_Size;
	cout<<endl;
	...
	for(int process_num = 1	; process_num <= 16 ; process_num++){
		...
		for(int i=0 ; i < 16 ; i++){
			waitpid(child_pcs[i],&status,0);
		}
		
		
		
		if(child_pcs[15]==0 && process_num >15){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,15,process_num);
			checksum[0] =  checksum[0] + temp_ ;
			exit(0);
		}
		if(child_pcs[14]==0 && process_num >14){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,14,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[13]==0 && process_num >13){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,13,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[12]==0 && process_num >12){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,12,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[11]==0 && process_num >11){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,11,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[10]==0 && process_num >10){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,10,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[9]==0 && process_num >9){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,9,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[8]==0 && process_num >8){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,8,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[7]==0 && process_num >7){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,7,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[6]==0 && process_num >6){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,6,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[5]==0 && process_num >5){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,5,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[4]==0 && process_num >4){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,4,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[3]==0 && process_num >3){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,3,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[2]==0 && process_num >2){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,2,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		}
		if(child_pcs[1]==0 && process_num >1){
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,1,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);
		...
		if(child_pcs[0] == 0){
			...
			temp_ = MatrixComputation(Matrix_A, Matrix_B, Matrix_C, Matrix_Size,0,process_num);
			checksum[0] =  checksum[0] + temp_;
			exit(0);	
		}
	...
	shmctl(shmid_checksum , IPC_RMID ,NULL);
